...
class FCDenseNet(nn.Module):
    def __init__(
        ...
        self.add_module(
            "bottleneck",
            Bottlenect(cur_channels_count, growth_rate, bottleneck_layers, dropout),
        )
        ...
        self.denseBlocksUp.append(
            DenseBlock(
                cur_channels_count, growth_rate, up_blocks[-1], dropout, upsample=False
            )
        )
        ...
        self.finalConv = nn.Conv2d(
            cur_channels_count, n_classes, kernel_size=1, stride=1, padding=0, bias=True
        )
        self.softmax = nn.Softmax(dim=1)
        self.log_var = nn.Conv2d(
            cur_channels_count, 1, kernel_size=1, stride=1, padding=0, bias=True
        )
