...
class DenseLayer(nn.Sequential):
    ...
    def __init__(self, in_channels: int, growth_rate: float, dropout=0.5) -> None:
        super().__init__()
...
class TransitionUp(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        ...
        self.convTrans = nn.ConvTranspose2d(
            in_channels, out_channels, kernel_size=3, stride=2, padding=0, bias=True
        )
...
class Bottlenect(nn.Sequential):
    def __init__(self, in_channels, growth_rate, n_layers, dropout):
        ...
        self.add_module(
            "bottleneck",
            DenseBlock(in_channels, growth_rate, n_layers, dropout, upsample=True),
        )
